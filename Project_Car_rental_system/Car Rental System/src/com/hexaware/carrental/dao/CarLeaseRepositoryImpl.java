...
public class CarLeaseRepositoryImpl implements ICarLeaseRepository{
	...
	public int addCar(Vehicle vehicle) throws DuplicateDataException,SQLException{
		int count=0;
		String insert="insert into vehicle(vehicleId,make,model,year,dailyRate,status, passangerCapacity ,engineCapacity) values(?,?,?,?,?,?,?,?)";
		try {
       ...
       List<Vehicle> list=new ArrayList<Vehicle>();
		...
		try {
			...
			while(rs.next()) {
				...
				int passengerCapacity=rs.getInt("PassangerCapacity");
	...
	public List<Vehicle> listRentedCars() {
		// TODO Auto-generated method stub
		 List<Vehicle> list=new ArrayList<Vehicle>();
			...
			String selectAll="select * from vehicle where status='NotAvailable' ";
			try {
				PreparedStatement pstmt=conn.prepareStatement(selectAll);
